// src/components/LatestSongs.jsx
...
import { fetchLatestSongs, getSongStreamUrl, getSongDownloadUrl } from '../services/api';
...
const LatestSongs = ({ limit = 6 }) => {
  ...
  useEffect(() => {
    ...
    const loadLatestSongs = async () => {
      try {
        setLoading(true);
        const data = await fetchLatestSongs(limit);
        setSongs(data);
      } catch (error) {
  ...
  // Check if a song is less than 7 days old
  ...
  if (loading) {
    return (
      <Box sx={{ my: 4 }}>
        <SectionTitle variant="h5">Latest Releases</SectionTitle>
